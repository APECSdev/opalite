...
import { type AgeVerificationPrivateState, witnesses } from '../../packages/contracts/src/witnesses';
...
import {
  type AgeVerificationCircuits,
  type AgeVerificationContract,
  AgeVerificationPrivateStateId,
  type AgeVerificationProviders,
  type DeployedAgeVerificationContract,
} from './common-types';
...
// Pre-compile the age verification contract with ZK circuit assets
const ageVerificationCompiledContract = CompiledContract.make('age_verification', AgeVerification.Contract).pipe(
...
export const getAgeVerificationLedgerState = async (
  ...
  const state = await providers.publicDataProvider
    ...
    .then((contractState) => (contractState != null ? AgeVerification.ledger(contractState.data).verifiedCount : null));
  logger.info(`Ledger state: ${state}`);
  return state;
};

export const ageVerificationContractInstance: AgeVerificationContract = new AgeVerification.Contract(witnesses);

export const joinContract = async (
  providers: AgeVerificationProviders,
  contractAddress: string,
): Promise<DeployedAgeVerificationContract> => {
  const ageVerificationContract = await findDeployedContract(providers, {
    contractAddress,
    compiledContract: ageVerificationCompiledContract,
    privateStateId: 'ageVerificationPrivateState',
    initialPrivateState: { privateCounter: 0 },
  });
  logger.info(`Joined contract at address: ${ageVerificationContract.deployTxData.public.contractAddress}`);
  return ageVerificationContract;
};

export const deploy = async (
  providers: AgeVerificationProviders,
  privateState: AgeVerificationPrivateState,
): Promise<DeployedAgeVerificationContract> => {
  logger.info('Deploying age verification contract...');
  const ageVerificationContract = await deployContract(providers, {
    compiledContract: ageVerificationCompiledContract,
    privateStateId: 'ageVerificationPrivateState',
    initialPrivateState: privateState,
  });
  logger.info(`Deployed contract at address: ${ageVerificationContract.deployTxData.public.contractAddress}`);
  return ageVerificationContract;
};
...
export const displayVerifiedCount = async (
  providers: AgeVerificationProviders,
  ageVerificationContract: DeployedAgeVerificationContract,
): Promise<{ verifiedCount: bigint | null; contractAddress: string }> => {
  const contractAddress = ageVerificationContract.deployTxData.public.contractAddress;
  const verifiedCount = await getAgeVerificationLedgerState(providers, contractAddress);
  if (verifiedCount === null) {
    logger.info(`There is no age verification contract deployed at ${contractAddress}.`);
  } else {
    logger.info(`Current verified count: ${Number(verifiedCount)}`);
  }
  return { contractAddress, verifiedCount };
};
...
${DIV}
  ...
  Dust
  └─ Address: ${DustAddress.encodePublicKey(networkId, state.dust.publicKey)}
...
export const configureProviders = async (ctx: WalletContext, config: Config) => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<AgeVerificationCircuits>(contractConfig.zkConfigPath);
